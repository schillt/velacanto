...
#else
    /// Retains app-lifetime composition when building with an SDK that predates
    /// the OS 27 Now Playing framework. The native media session is compiled and
    /// activated whenever the framework is available.
    @MainActor
    final class PlaybackSystemMediaSession: ObservableObject {
        let id = "com.chameleonenterprise.velacanto.playback"

        init(playback _: AudioPlaybackCoordinator) {}
