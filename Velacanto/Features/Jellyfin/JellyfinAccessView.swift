    ...
    private var signedInView: some View {
        List {
            ...
            Section("Music Libraries") {
                if jellyfin.isRefreshingLibraries {
                    ProgressView("Loading music libraries…")
                    ...
                    ContentUnavailableView(
                        ...
                        description: Text(
                            "No accessible Jellyfin music library was returned. "
                                + "Pull to refresh or check this user's library access."
                        )
                    )
                } else {
                    ForEach(jellyfin.libraries) { library in
                        NavigationLink {
                            ...
                            Label(library.name, systemImage: "music.note.house.fill")
                        }
                    }
                }
            }
        ...
        .refreshable {
            await jellyfin.refreshLibraries()
        }
    }
}
...
private struct JellyfinAlbumsView: View {
        ...
        List {
            if isLoading {
                ...
                ContentUnavailableView(
                    ...
                    description: Text("This library did not return any music albums.")
                )
            } else {
    ...
    private func load() async {
        ...
        errorMessage = nil
        do {
            albums = try await jellyfin.albums(in: library)
        } catch {
            albums = []
